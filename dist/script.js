...
/***/ function(module, exports, __webpack_require__) {
	...
	var _getEl = __webpack_require__(5);

	var _getEl2 = _interopRequireDefault(_getEl);

	var _Place = __webpack_require__(6);

	var _Place2 = _interopRequireDefault(_Place);

	var _PlaceArchive = __webpack_require__(7);

	var _PlaceArchive2 = _interopRequireDefault(_PlaceArchive);
	...
	var MapApp = function () {
		function MapApp() {
			...
			var template = (0, _getEl2.default)('#template-map').innerHTML;
			var compiled = _.template(template);
			var newElement = compiled();

			var mapTypeId = google.maps.MapTypeId;
			var selector = config.selector;
			var _config$mapType = config.mapType;
			var mapType = _config$mapType === undefined ? 'ROADMAP' : _config$mapType;
			var _config$zoom = config.zoom;
			var zoom = _config$zoom === undefined ? 13 : _config$zoom;
			var _config$center = config.center;
			var center = _config$center === undefined ? { lat: 46.488056, lan: 30.741111 } : _config$center;
			var iconFavorite = config.iconFavorite;
			...
			this.$rootEl = selector ? (0, _getEl2.default)(selector) : document.body;
			this.$rootEl.insertAdjacentHTML('afterbegin', newElement);

			this.$mapEl = (0, _getEl2.default)('.map', this.$rootEl);
			this.$placeList = (0, _getEl2.default)('.place-list', this.$rootEl);
			this.newPlaceHeight = (0, _getEl2.default)('.new-place', this.$rootEl).offsetHeight;
		...
		_createClass(MapApp, [{
			...
			value: function checkLS() {
				...
				if (localStorage[this.myPlaces.selector]) {
					_.forEach(JSON.parse(localStorage[this.myPlaces.selector]), function (item) {
						var place = new _Place2.default(item.name, item.latitude, item.longitude, item.inFavorite);
						_this.myPlaces.push(place);
					});
				}
			}
		}, {
			...
			value: function mapInit() {
				...
				var mapLoad = function mapLoad(lat, lan) {
					_this2.map = new google.maps.Map(_this2.$mapEl, {
						center: new google.maps.LatLng(lat, lan),
						zoom: _this2.zoom,
						mapTypeId: _this2.mapType,
				...
				var geoSuccess = function geoSuccess(position) {
					...
					_this2.myPlaces = new _PlaceArchive2.default(_this2.map, _this2.selector, _this2.$rootEl, _this2.iconFavorite);
					_this2.checkLS();
				};

				var geoFailure = function geoFailure() {
					// alert("Ошибка геолокации");
					mapLoad(_this2.center.lat, _this2.center.lan);
					_this2.myPlaces = new _PlaceArchive2.default(_this2.map, _this2.selector, _this2.$rootEl, _this2.iconFavorite);
					_this2.checkLS();
				};
			...
			value: function eventsInit() {
				...
				(0, _getEl2.default)('.add', this.$rootEl).addEventListener('click', function () {
					var name = (0, _getEl2.default)('.new-place input', _this3.$rootEl).value;
					if (name.length > 0) {
						(0, _getEl2.default)('.new-place input', _this3.$rootEl).value = '';
						(0, _getEl2.default)('.helper', _this3.$rootEl).classList.remove('hide');
						google.maps.event.addListenerOnce(_this3.map, 'click', function (e) {
							var place = new _Place2.default(name, e.latLng.lat(), e.latLng.lng());
							_this3.myPlaces.push(place);
							(0, _getEl2.default)('.helper', _this3.$rootEl).classList.add('hide');
						});
					} else {
				...
				this.$placeList.addEventListener('click', function (e) {
					var place = e.target.closest('.place');
					var num = _.indexOf(place.parentNode.children, place);

					if (e.target.matches('.favorite')) {
						var $favButton = place.querySelector('.favorite');
						$favButton.classList.toggle('fa-star-o');
						$favButton.classList.toggle('fa-star');
						_this3.myPlaces.inFavorite(num);
					}
					if (e.target.matches('.delete')) {
						place.remove();
						_this3.myPlaces.removePlace(num);
					}
					...
					if (e.target.matches('.save')) {
						...
						_this3.myPlaces.saveEdit(num, place);
						// } else{
						// 	alert('Введите текст заметки')
						// }
					}
				...
				this.$placeList.addEventListener('mouseover', function (e) {
					return _this3.listHover(e, google.maps.Animation.BOUNCE, _this3.myPlaces);
				});
				this.$placeList.addEventListener('mouseout', function (e) {
					return _this3.listHover(e, null, _this3.myPlaces);
				});
				// Показ/скрытие списка
				(0, _getEl2.default)('.open-list', this.$rootEl).addEventListener('click', function () {
					return _this3.$placeList.classList.toggle('hide');
				});
				// Изменение размера карты
				window.addEventListener('resize', function () {
					return _this3.setWindowSize();
				});
			}
		}, {
			key: 'setWindowSize',
			value: function setWindowSize() {
				var $rootEl = this.$rootEl === document.body ? window : this.$rootEl;

				var $rootElHeight = $rootEl === window ? $rootEl.innerHeight : $rootEl.offsetHeight;
			...
			key: 'listHover',
			value: function listHover(e, animation) {
				if (e.target.matches('.place p')) {
					var num = _.indexOf(e.target.closest('.place-list').children, e.target.parentNode);
...
/***/ function(module, exports) {
	...
	exports.default = function (selector, scope) {
	  return scope === window ? document.querySelector(selector) : (scope || document).querySelector(selector);
	};
...
/***/ function(module, exports) {
	...
	var Place = function Place(name, latitude, longitude) {
		var fav = arguments.length <= 3 || arguments[3] === undefined ? false : arguments[3];
...
/***/ function(module, exports, __webpack_require__) {
	...
	var PlaceArchive = function (_ArrayProx) {
		_inherits(PlaceArchive, _ArrayProx);

		function PlaceArchive(map, selector, $rootEl, iconFavorite) {
			_classCallCheck(this, PlaceArchive);

			var _this = _possibleConstructorReturn(this, Object.getPrototypeOf(PlaceArchive).call(this));

			_this.map = map;
			_this.selector = selector === undefined ? 'myPlaces' : 'myPlaces' + selector;
			_this.$rootEl = $rootEl;
			_this.iconFavorite = iconFavorite ? iconFavorite : 'styles/imgs/marker-favorite.png';
			return _this;
		}

		_createClass(PlaceArchive, [{
			key: 'savePlaces',
			value: function savePlaces() {
				var savedPlaces = [];
			...
			value: function push(place) {
				_get(Object.getPrototypeOf(PlaceArchive.prototype), 'push', this).call(this, place);
				this.renderItem(place);
				this.savePlaces();
			}
		}, {
			key: 'inFavorite',
			value: function inFavorite(num) {
			...
			key: 'removePlace',
			value: function removePlace(num) {
			...
			key: 'renderItem',
			value: function renderItem(place) {
				var template = (0, _getEl2.default)('#template-place').innerHTML;
				var compiled = _.template(template);
				var newElement = compiled(place);
				...
				(0, _getEl2.default)('.place-list', this.$rootEl).insertAdjacentHTML('beforeEnd', newElement);
			}
		}, {
			key: 'saveEdit',
			value: function saveEdit(num, place) {
